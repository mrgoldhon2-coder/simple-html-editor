...
const SearchableDropdown = ({ 
  ...
  // Ref для мгновенной фиксации выбора (чтобы handleBlur видел это без задержек стейта)
  ...
  // Логика автовыбора при вводе 3-х символов
  useEffect(() => {
    if (filtered.length === 1 && search.length >= 3) {
      const selected = filtered[0];
      isSelectionMade.current = true; // Мгновенно помечаем, что выбор сделан
  ...
  const handleBlur = () => {
    setTimeout(() => {
      // Если выбор подтвержден (клик или автовыбор), ничего не сбрасываем
      ...
      // Откат к предыдущему значению, если ничего не выбрано или введено некорректно
        ...
        {mode === 'login' ? (
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="password" placeholder="Введите пароль" className="w-full bg-[#0f1419] border border-[#2a3040] rounded-xl px-4 py-3 focus:border-[#FDB913] focus:outline-none transition" />
            </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-sm font-medium mb-2 block">Пароль</label>
              <input type="password" placeholder="Создайте пароль" className="w-full bg-[#0f1419] border border-[#2a3040] rounded-xl px-4 py-3 focus:border-[#FDB913] focus:outline-none transition" />
            </div>
            <div>
              <label className="text-sm font-medium mb-2 block">Подтвердите пароль</label>
              <input type="password" placeholder="Повторите пароль" className="w-full bg-[#0f1419] border border-[#2a3040] rounded-xl px-4 py-3 focus:border-[#FDB913] focus:outline-none transition" />
            </div>
            <button className="w-full bg-[#FDB913] text-black py-3 rounded-xl font-semibold hover:bg-[#E5A712] transition mt-6">
              Зарегистрироваться
            </button>
