...
/**
 * ВЫПАДАЮЩИЙ СПИСОК
 * Нативная версия: только поиск и выбор, без вмешательства в скролл.
 */
  ...
  return (
    <div className="relative w-full">
      <input
        ...
        className={`input-base ${isOpen ? 'relative z-[100] border-[#FDB913] bg-[#1a1f26]' : ''}`}
      />
      {isOpen && (
        <>
...
const Navbar = ({ page, setPage }: { page: Page; setPage: (p: Page) => void }) => {
  ...
  return (
    <header className="w-full bg-black border-b border-[#1a1f26] sticky top-0 z-[100]">
  ...
  return (
    <div className="page-container py-12">
      ...
      <div className="card-dark space-y-8 p-6 md:p-10 bg-[#11141a] rounded-3xl border border-white/5">
        ...
        <button onClick={handleCreateOrder} disabled={!isValid || loading} className="btn-secondary w-full md:w-96 mx-auto h-16 flex items-center justify-center gap-3 font-bold">
  ...
  <div className="page-container py-12">
    ...
    <div className="flex gap-4 mb-8 overflow-x-auto pb-2 no-scrollbar">
      {Object.entries(RU.profile.tabs).map(([k, v]) => (
        <button key={k} className={`px-6 py-2 rounded-full text-sm font-medium whitespace-nowrap ${k === 'all' ? 'bg-[#FDB913] text-black' : 'bg-[#1a1f26] text-white'}`}>{v}</button>
      ))}
    </div>
...
const RewardsPage = () => (
  <div className="page-container py-12">
    ...
    <p className="text-[#6B7280] mb-10">{RU.rewards.subtitle}</p>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
      {RU.rewards.stats.map((s, i) => (
        <div key={i} className="p-8 bg-[#1a1f26] rounded-3xl border border-white/5">
          ...
          <div className="text-xs text-[#6B7280] font-bold uppercase mb-1">{s.l}</div>
    ...
    <div className="space-y-4">
      {RU.rewards.items.map((item, i) => (
        <div key={i} className="flex items-center justify-between p-6 bg-[#1a1f26] rounded-2xl border border-white/5">
          <div><h3 className="font-bold text-white text-sm">{item.t}</h3><p className="text-xs text-[#6B7280]">{item.d}</p></div>
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <footer className="py-12 border-t border-[#1a1f26] text-center text-[10px] text-[#4B5563] uppercase tracking-widest">
